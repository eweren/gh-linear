...
import { arrayRotate } from '../../shared/utils';
...
function SelectInput<V>({
	...
	useEffect(() => {
		if (previousItems.current.map(item => item.value).every((v, i) => items[i]?.value === v)) {
			setRotateIndex(0);
			setSelectedIndex(0);
		}
	...
	useInput(
		useCallback(
          ...
          const newHighlight = slicedItems[nextSelectedIndex];
					if (typeof onHighlight === 'function' && newHighlight) {
          ...
          const newHighlight = slicedItems[nextSelectedIndex];
            ...
            const newHighlight = slicedItems[selectedIndex];
          if (typeof onHighlight === 'function' && newHighlight) {
	...
	return (
		<Box flexDirection="column">
			{slicedItems.map((item, index) => {
				...
				return (
					<Box key={item.key ?? (typeof item.value === "string" ? item.value : JSON.stringify(item.value) as string)}>
